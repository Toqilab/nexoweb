...
const registrarHistorialActividad = async ({
  ...
  if (!tarea?.id || !tarea?.acuario_id) return
  ...
  if (error) {
    console.warn(
      'No se pudo guardar el historial de la actividad:',
      error
    )
  }
}

const horaBonita = (valor) => {
    ...
    try {
      ...
      if (tipo === 'medicacion') {
        ...
        if (duracion > 1) {
          ...
          await registrarHistorialActividad({
            tarea,
            accion: 'creada',
            estadoNuevo: form.estado,
            fechaNueva: fechaProgramada,
            detalle: 'Medicación creada.',
          })
      ...
      if (
        ...
        await registrarHistorialActividad({
          ...
          estadoNuevo: form.estado,
          fechaNueva: fechaProgramada,
          detalle: 'Actividad creada manualmente.',
        })
        ...
        if (
          ...
          await registrarHistorialActividad({
            ...
            estadoNuevo: form.estado,
            fechaNueva: fechaProgramada,
            detalle: 'Primera ocurrencia de una actividad recurrente.',
          })
...
export function CalendarioActividades({
  ...
  const materializarEvento = async (evento) => {
    if (evento?.tarea?.id) {
      return evento.tarea
    }

    if (evento?.origen !== 'rutina' || !evento?.rutina?.id) {
      return null
    }
    ...
    const metadata =
      evento.rutina.metadata ||
      {}

    let numeroDosis = null
    let totalDosis = null
    let titulo = evento.titulo

    if (
      evento.tipo === 'medicacion' &&
      metadata.total_dosis
    ) {
      numeroDosis =
        Math.floor(
          diferenciaDias(
            evento.rutina.fecha_inicio,
            evento.fecha
          ) /
          Math.max(
            1,
            Number(evento.rutina.intervalo) || 1
          )
        ) + 1

      totalDosis =
        Number(metadata.total_dosis)

      titulo =
        `${evento.rutina.titulo} · ` +
        `Dosis ${numeroDosis} de ${totalDosis}`
    }
      ...
      .insert([
        {
          ...
          rutina_id: evento.rutina.id,
          fecha_rutina: evento.fecha,
          tratamiento_id:
            metadata.tratamiento_id || null,
          numero_dosis: numeroDosis,
          total_dosis: totalDosis,
          titulo,
          tipo: evento.tipo,
          descripcion:
            evento.rutina.descripcion || null,
          fecha_programada: fechaProgramada,
          fecha_original: fechaProgramada,
          estado: 'pendiente',
          recordatorio_minutos:
            evento.rutina.recordatorio_minutos || null,
          producto_id:
            evento.rutina.producto_id || null,
          regla_dosificacion_id:
            evento.rutina.regla_dosificacion_id || null,
          aplicar_sobre:
            evento.rutina.aplicar_sobre || null,
          volumen_litros:
            evento.rutina.litros ?? null,
          metadata,
        },
      ])
      .select()
    ...
    if (error) {
      if (
        ...
        const { data: existente } = await supabase
          .from('tareas_acuario')
          .select('*')
          .eq('rutina_id', evento.rutina.id)
          .eq('fecha_rutina', evento.fecha)
          .maybeSingle()
    ...
    await registrarHistorialActividad({
      ...
      fechaNueva: fechaProgramada,
      detalle: 'Ocurrencia generada desde una rutina.',
    })
  ...
  const eventos = useMemo(() => {
    ...
    while (cursor <= finGrilla) {
      ...
      for (const rutina of rutinas) {
        if (!rutinaOcurre(rutina, fechaTexto)) continue

        const tareaMaterializada = tareas.find(
          (tarea) =>
            tarea.rutina_id === rutina.id &&
            tarea.fecha_rutina === fechaTexto
        )

        if (tareaMaterializada) continue

        const metadata = rutina.metadata || {}

        let titulo = rutina.titulo
  ...
  const colorEvento = (evento) => {
    ...
    if (evento.estado === 'omitida') return 'omitida'

    if (
      ['cambio_agua', 'medicion_agua'].includes(
        evento.tipo
      )
    ) {
      return 'azul'
    ...
    if (
      ['mantenimiento', 'limpieza', 'fertilizacion'].includes(
        evento.tipo
      )
    ) {
      return 'verde'
  ...
  const cargarHistorial = async (tareaId) => {
    if (!tareaId) {
      setHistorial([])
      return
    }
  ...
  const abrirDetalle = async (evento) => {
    ...
    if (evento?.tarea?.id) {
      await cargarHistorial(evento.tarea.id)
    } else {
      setHistorial([])
    }
  }

  const abrirAccion = async (evento, accion) => {
    try {
      const tarea = await materializarEvento(evento)

      if (!tarea?.id) {
        onMensaje?.('No se pudo preparar esta actividad.')
        return
      }
      ...
      setFormAccion({
        fecha:
          accion === 'reprogramar'
            ? fechaLocal(
                new Date(tarea.fecha_programada)
              )
            : fechaLocal(),
        hora:
          accion === 'reprogramar'
      ...
      onMensaje?.(`❌ ${error.message}`)
    }
  }

  const ejecutarAccion = async () => {
    const tarea = eventoDetalle?.tarea

    if (!tarea?.id || !accionModal) return
    ...
    try {
      ...
      if (accionModal === 'completar') {
        ...
        const { data, error } = await supabase
          ...
          .update({
            estado: 'completada',
            completada_en: fechaRealizada,
            observacion_final:
              valorONull(
                formAccion.observacion
              ),
            updated_at:
              new Date().toISOString(),
          })
        ...
        await registrarHistorialActividad({
          ...
          fechaAnterior: tarea.fecha_programada,
          fechaNueva: fechaRealizada,
          detalle:
            valorONull(formAccion.observacion) ||
            'Actividad completada.',
        })

        onMensaje?.('✅ Actividad completada.')
      }

      if (accionModal === 'omitir') {
        const { data, error } = await supabase
          .from('tareas_acuario')
          .update({
            estado: 'omitida',
        ...
        await registrarHistorialActividad({
          ...
          fechaAnterior: tarea.fecha_programada,
          detalle:
            valorONull(formAccion.motivo) ||
            'Actividad omitida.',
        })

        onMensaje?.('Actividad omitida.')
      }

      if (accionModal === 'reprogramar') {
        const nuevaFecha =
          fechaHoraAISO(
            formAccion.fecha,
            formAccion.hora || '09:00'
          )
        ...
        await registrarHistorialActividad({
          ...
          detalle:
            `Actividad reprogramada de ` +
            `${fechaBonita(fechaAnterior)} a ` +
            `${fechaBonita(nuevaFecha)}.`,
        })

        onMensaje?.('✅ Actividad reprogramada.')
      }
  ...
  return (
    <div>
      ...
      {eventoDetalle && (
        ...
        >
          <div
            className="modal-actividad-detalle modal-actividad-detalle-final"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
            ...
            <div className="acciones-actividad-final">
              {eventoDetalle.estado !==
                'completada' && (
                <button
                  className="boton-principal"
            ...
            <div className="historial-actividad-final">
              <h3>Historial</h3>

              {!eventoDetalle.tarea?.id ? (
                <p>
                  La ocurrencia todavía pertenece a una regla recurrente
                  y se materializará cuando se gestione.
                </p>
              ) : cargandoHistorial ? (
                <p>Cargando...</p>
              ) : historial.length === 0 ? (
            ...
            <button
              className="boton-cancelar boton-ancho"
              onClick={() =>
                setEventoDetalle(null)
              }
            >
      ...
      {accionModal && eventoDetalle?.tarea && (
        <div
          className="modal-overlay"
          onClick={() =>
            !guardandoAccion &&
        ...
        >
          <div
            className="modal-confirmacion modal-accion-actividad"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
